#!/usr/bin/env python3
"""
Generate comprehensive Jenkins documentation content
Completes all remaining intermediate, advanced, and lab files
"""
...
templates = {
    "interview_intermediate": """# Jenkins Intermediate - Interview Questions
...
def main():
    # Create interview questions file
    filepath = "intermediate/interview-questions-intermediate.md"
    
    # Backup if exists
    if os.path.exists(filepath):
        with open(filepath, 'r') as f:
            existing_lines = len(f.readlines())
        print(f"Existing file has {existing_lines} lines")
    
    with open(filepath, 'w') as f:
        f.write(templates["interview_intermediate"])
    
    print(f"Created/Updated: {filepath}")
    with open(filepath, 'r') as f:
        lines = len(f.readlines())
    print(f"New file has {lines} lines")
